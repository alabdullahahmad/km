                    ...
                    <div class="card-body p-30">
                        <div class="service-man-list">
                            ...
                            @php
                                ...
                                $is_ready_to_receive = $handyman->amount_status == 'ready'; // افتراض أن `amount_status` هي الحقل الذي يحتوي على حالة المبلغ
                            @endphp
                            <div class="service-man-list__item">
                                ...
                                <div class="service-man-list__item_body">
                                    <p class="service-man-amount">{{ __('messages.amount_due') }}: {{ $handyman->amount_due ?? '29034082304932' }}</p>
                                    @if ($is_ready_to_receive)
                                        <p style="color: green;">{{ __('messages.ready_to_receive') }}</p>
                                        <button class="btn btn-success">{{ __('messages.receive') }}</button>
                                    @else
